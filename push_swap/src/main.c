...
/*   Updated: 2024/02/17 13:48:54 by bpoyet           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static t_listx *ft_fill_list1(char *argv)
{
	...
	while (i >= 0)
	{
		ft_insert_front(lista, ft_atoi(split[i]));
		i--;
	}
	free_twodim(split);
	return (lista);
...
int main(int argc, char **argv)
{
	...
	if (ft_check_if_sort(lista))
		...
		algo_five(lista);
	else if (ft_size_listx(lista) > 5 && ft_size_listx(lista) <= 300)
		algo_cent(lista);
	else if (ft_size_listx(lista) > 300)
		algo_cinqcent(lista);
	ft_free_list(lista);
	return (0);
}
